...
recall = recall_score(training_data['Label'], training_data['AI_Predicted'], average=None, labels=le.classes_, zero_division=0)
f1 = f1_score(training_data['Label'], training_data['AI_Predicted'], average=None, labels=le.classes_, zero_division=0)

# Extract precision, recall, and F1 score for Positif and Negatif classes
positif_precision = precision[le.transform(['Positif'])[0]]
negatif_precision = precision[le.transform(['Negatif'])[0]]
positif_recall = recall[le.transform(['Positif'])[0]]
negatif_recall = recall[le.transform(['Negatif'])[0]]
positif_f1 = f1[le.transform(['Positif'])[0]]
negatif_f1 = f1[le.transform(['Negatif'])[0]]

print(f"Precision for Positif class: {positif_precision:.5f}")
print(f"Precision for Negatif class: {negatif_precision:.5f}")
print(f"Recall for Positif class: {positif_recall:.5f}")
print(f"Recall for Negatif class: {negatif_recall:.5f}")
print(f"F1 Score for Positif class: {positif_f1:.5f}")
print(f"F1 Score for Negatif class: {negatif_f1:.5f}")

# Generate classification report
report = classification_report(training_data['Label'], training_data['AI_Predicted'], target_names=le.classes_, zero_division=0)
...
# Scatter plot to visualize predictions vs true labels
plt.figure(figsize=(10, 6))

# Map true labels and predicted labels to integers for plotting
true_labels_int = le.transform(training_data['Label'])
predicted_labels_int = le.transform(training_data['AI_Predicted'])

# Scatter plot
plt.scatter(range(len(true_labels_int)), true_labels_int, color='blue', alpha=0.5, label='True Labels')
plt.scatter(range(len(predicted_labels_int)), predicted_labels_int, color='red', alpha=0.5, label='Predicted Labels')
